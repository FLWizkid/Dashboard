...
- `priorities` — the first data module (per-user list).
- **Row Level Security is enabled on every table**; users can only read/write
  their own rows (`auth.uid() = user_id`).

## Note

The `priorities` schema is a first-pass reconstruction. Refine the columns when
the detailed module design lands — see [`../PLAN.md`](../PLAN.md).
