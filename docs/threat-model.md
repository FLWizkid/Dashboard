...
| R3  | ~~**No audit log**~~ — **closed 2026-08-20**                      | Now implemented: `audit_log` is append-only (select + insert policies only, no update or delete), holds identifiers and verbs but never subjects, bodies or addresses, and ages out after 24 months alongside the mail it describes. Written from the sync and credential paths; `recordAudit()` reports rather than throws, trading log completeness for product availability.         | **P7**                                            |
