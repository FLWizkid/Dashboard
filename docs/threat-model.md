...
| **OAuth tokens** (P2)                | Not data but _access_ — a token is a standing key to the live Google or Microsoft account           | `db-data`, encrypted at the field level       |
...
| R4  | **No retention/purge job** (spec default: 24 months)           | Nothing yet accumulates at a rate that needs it; mail does                                                                     | **P2/P7**                        |
| R5  | **Mail bodies not yet field-encrypted**                        | There are no mail bodies yet. `pgcrypto` is installed at bootstrap precisely so this is a schema change, not a stack change    | **P2**                           |
...
| ------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **P2 — Email, calendar** | The biggest shift in the project. Third-party HTML gets rendered (R1 must close), OAuth tokens become a stored asset, mail bodies need field encryption (R5), retention starts to matter (R4) |
